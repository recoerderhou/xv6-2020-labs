...
found:
  ...
  p->kernel_pagetable = new_kvminit();
  char *pa = kalloc();
  if(pa == 0)
  	panic("kalloc");
  // third highest
  ...
  p->kstack = va;
  // printf("kstack allocation: ");
  // printf("%d", walk(p->kernel_pagetable, va, 0));
...
{	
  ...
  if(p->kernel_pagetable)
  	proc_freekernelpgtbl(p->kernel_pagetable, p->kstack, p->sz);
...
userinit(void)
{
  ...
  kernel_pte = walk(p->kernel_pagetable, 0, 1);
