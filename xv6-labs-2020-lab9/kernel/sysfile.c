...
sys_mmap(void)
{
  ...
  if((argaddr(0, &addr) < 0) || (argint(1, &length) < 0) || (argint(2, &prot) < 0) || (argint(3, &flags) < 0) || (argfd(4, &fd, &mfile) < 0) || (argint(5, &offset) < 0))
    return mmap_error;
  ...
  for(int i = 0; i < 16; ++ i){
    if((p->vmas[i].addr == addr) || (p->vmas[i].addr + p->vmas[i].length == addr + length)){
      ...
      if((p->vmas[i].flags & MAP_SHARED) && (p->vmas[i].prot & PROT_WRITE)){
        ...
        filewrite(p->vmas[i].mfile, addr, length);
      }
